...
function MessageQueue() {
  ...
  self.enqueueAndProcess = function (type, message) {
    ...
    return _enqueue(queueItem).then(function () {
    ...
    if (_numWorkers < self.maxWorkers) {
      ...
      _receive()
        .then(function (queueItem) {
          if (queueItem) {
            return _process(queueItem).then(function () {
  ...
  function _process(queueItem) {
    ...
    return worker(queueItem).then(function (status) {
      switch (status) {
        case 'Completed':
  ...
  function _receive() {
    ...
    return self.databasePromise().then(function (db) {
      return db
        ...
        .findOneAndUpdate(query, update, {returnDocument: 'after', sort: 'priority'});
    });
  }
