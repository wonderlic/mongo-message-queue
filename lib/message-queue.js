...
function MessageQueue() {
  ...
  self.dequeue = function(type, messageQuery) {
    var query = _buildQueueItemQuery(type, messageQuery);

    return _dequeueByQuery(query);
  };

  self.update = function(type, messageQuery, messageUpdate) {
    var query = _buildQueueItemQuery(type, messageQuery);
    var update = _buildQueueItemUpdate(messageUpdate);
  ...
  function _dequeue(queueItem) {
    return _dequeueByQuery({_id: queueItem._id});
  }

  function _dequeueByQuery(query) {
    if (!self.databasePromise) { return Q.reject(new Error("No database configured")); }
    return self.databasePromise()
      .then(function(db) {
  ...
  function _buildQueueItemUpdate(messageUpdate) {
    var update = {$set: {}};
