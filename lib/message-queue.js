...
function MessageQueue() {
  ...
  async function _update(query, update, multi) {
    ...
    if (multi) {
      ...
      result = await collection.updateMany(query, update);
    }
  ...
  async function _reject(queueItem) {
    ...
    const result = await collection.updateOne({_id: queueItem._id}, update);
    return result.modifiedCount;
  }
  ...
  async function _receive() {
    ...
    const result = await collection.findOneAndUpdate(query, update, {returnOriginal: false, sort: 'priority'});
    return result.value;
  }
  ...
  async function _getCollection() {
    ...
    const db = await self.databasePromise();
    return await db.collection(self.collectionName);
  }
