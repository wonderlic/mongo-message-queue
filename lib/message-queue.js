...
function MessageQueue() {
  ...
  self.autoCreateIndex = true;
  self.indexName = 'queueReceiveIndex';
  ...
  self.registerWorker = function (type, promise) {
    ...
    if (self.autoCreateIndex) {
      self.ensureIndexes().catch(self.errorHandler);
    }
  ...
  self.ensureIndexes = function () {
    // repeated worker registrations only issue the createIndex once
    if (!_ensureIndexesPromise) {
      _ensureIndexesPromise = _ensureIndexes();
    }
  ...
  function _release(queueItem) {
    const update = {
      ...
      $set: {
        ...
        nextReceivableTime: queueItem.nextReceivableTime
          ? queueItem.nextReceivableTime
          : new Date(),
      },
      $push: {
        releaseHistory: {
      ...
      $and: [
        ...
        },
        {
          $or: [
            {
              receivedTime: {
                $lt: new Date(Date.now() - self.processingTimeout),
              },
            },
  ...
  async function _ensureIndexes() {
    try {
      ...
      // These keys and their order mirror the _receive query and its priority sort.
      ...
      const isIndexAlreadyExistsError =
        err && (err.code === INDEX_OPTIONS_CONFLICT || err.code === INDEX_KEY_SPECS_CONFLICT);
